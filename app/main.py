...
def log_request(
        ...
        log_entry.update(
            {
                ...
                if k not in ("content", "file_content", "data", "password", "token")
            }
        )
    ...
    if settings.logging.to_s3:
        try:
            ...
            if hasattr(registry, "_backend") and hasattr(
                registry._backend, "_s3_client"
            ):
...
def check_rate_limit(client_key: str) -> bool:
    ...
    _rate_limit_store[client_key] = [
        t for t in _rate_limit_store[client_key] if t > window_start
    ]
...
async def upload_file(
    ...
    try:
        ...
        if ext not in ALLOWED_EXTENSIONS:
            ...
            return JSONResponse(
                ...
                content=make_response(
                    ...
                    error_message=f"Unsupported file type: .{ext}. Allowed: {', '.join(ALLOWED_EXTENSIONS)}",
                ),
            )
        ...
        if size_mb > MAX_UPLOAD_SIZE_MB:
            ...
            return JSONResponse(
                ...
                content=make_response(
                    ...
                    status="error",
                    error_code="INVALID_INPUT",
                    error_message=f"File too large: {size_mb:.1f}MB. Max: {MAX_UPLOAD_SIZE_MB}MB",
                ),
            )
        ...
        log_request(
            ...
            extra={"sheets_count": sheets_count, "filename": filename},
            log_type="upload",
        )
...
async def list_datasets(
    client_id: str = Query(...), x_request_id: Optional[str] = Header(None)
):
    request_id = x_request_id or generate_request_id()
    start_time = time.time()
    safe_client = normalize_client_id(client_id)
...
async def create_id(
    request: IDCreateRequest, x_request_id: Optional[str] = Header(None)
):
    request_id = x_request_id or generate_request_id()
    start_time = time.time()
    ...
    try:
        ...
        return make_response(
            data={"id": new_id, "user_id": safe_user}, request_id=request_id
        )
    except Exception as e:
        log_request(
            request_id,
...
async def list_ids(
    user_id: str = Query(...), x_request_id: Optional[str] = Header(None)
):
    request_id = x_request_id or generate_request_id()
    start_time = time.time()
...
async def validate_id(
    request: IDValidateRequest, x_request_id: Optional[str] = Header(None)
):
    request_id = x_request_id or generate_request_id()
    start_time = time.time()
    safe_user = normalize_client_id(request.user_id)
...
def run_cli():
    ...
    upload_parser.add_argument(
        "--ingest_all", action="store_true", help="Ingest all sheets"
    )
    ...
    if args.command == "upload":
        ...
        result = agent.execute_sql_query(
            query=args.query, df_id=dataset_id, client_id=args.client
        )
